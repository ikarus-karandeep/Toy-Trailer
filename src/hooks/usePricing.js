...
export function usePricing(ctx) {
  return useMemo(() => {
    ...
    if (ctx.cabinets) {
      ctx.cabinets.forEach(id => { appearance += findPrice(CABINET_OPTIONS, id); });
    }
    ...
    ctx.width, ctx.length, ctx.interiorHeight, ctx.axleAtp, ctx.axleSuspension, ctx.axleCapacity,
